...
namespace AnyBaseConversion
{
	...
	const boost::container::flat_map<AnyBaseConversion::Base,std::string> char_sets {
		{AnyBaseConversion::binary,"01"},
		{AnyBaseConversion::ternary,"012"},
		{AnyBaseConversion::octal,"01234567"},
		{AnyBaseConversion::decimal,"0123456789"},
		{AnyBaseConversion::docenal,"0123456789ab"},
		{AnyBaseConversion::hexadecimal,"0123456789abcdef"},
		{AnyBaseConversion::vigesimal,"0123456789abcdefghjk"},
		{AnyBaseConversion::base32,"abcdefghijklmnopqrstuvwxyz234567"},
		{AnyBaseConversion::base36,"0123456789abcdefghijklmnopqrstuvwxyz"},
		{AnyBaseConversion::base58,"123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"},
		{AnyBaseConversion::base64,"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"},
	};

	const boost::container::flat_map<std::string, AnyBaseConversion::Base> stringToBaseMap {
		{"binary",AnyBaseConversion::binary},
		{"ternary",AnyBaseConversion::ternary},
		{"octal",AnyBaseConversion::octal},
		{"decimal",AnyBaseConversion::decimal},
		{"docenal",AnyBaseConversion::docenal},
		{"hexadecimal",AnyBaseConversion::hexadecimal},
		{"vigesimal",AnyBaseConversion::vigesimal},
		{"base32",AnyBaseConversion::base32},
		{"base36",AnyBaseConversion::base36},
		{"base58",AnyBaseConversion::base58},
		{"base64",AnyBaseConversion::base64},
	};
	...
	public:
		...
		std::string output_with_new_base(Base base, bool use_uppercase = false)
		{
			defined_output_digits = true;
			convert_number_to_new_base(convert_Base_to_char_set(base));
			if (std::find(bases_that_allow_use_uppercase.begin(), bases_that_allow_use_uppercase.begin(), base) != bases_that_allow_use_uppercase.end())
				output_allows_use_uppercase = true;
			return output(use_uppercase);
		}
	...
	private:
		...
		bool																					is_float { false };
		bool																					output_allows_use_uppercase { false };
		...
		std::vector<Base>															bases_that_allow_use_uppercase { docenal, hexadecimal, vigesimal, base32, base36 };
		...
		{
			std::string char_set = AnyBaseConversion::char_sets.at(base);
		...
		void convert_output_digits_to_uppercase()
		{
		...
		std::string output(bool use_uppercase = false)
		{
			if (error != "")
			{
				return error;
				...
				if (use_uppercase && output_allows_use_uppercase)
					convert_output_digits_to_uppercase();
				else if (use_uppercase)
					set_error("Error: uppercase return was requested for a character set which does not allow it.");
				if (!is_float)
				{
